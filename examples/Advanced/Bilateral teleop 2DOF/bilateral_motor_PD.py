...
position_values_receive_int = np.array([0, 0, 0])
...
while True:
    ...
    if latest_data:
        ...
        if len(received_parts) > 5 and received_parts[0] == "pos":
            ...
            position_values_receive_int[0] = Joint[1].get_encoder_position(float(position_values_receive[0]))
            position_values_receive_int[1] = Joint[0].get_encoder_position(float(position_values_receive[1]))
            position_values_receive_int[2] = Joint[2].get_encoder_position(float(position_values_receive[2]))
            try:
    ...
    Motor[0].Send_data_pack_PD(Position = position_values_receive_int[1],Speed = speed_values_receive[1], Current = 0)
    Motor[1].Send_data_pack_PD(Position = position_values_receive_int[0],Speed = speed_values_receive[0], Current = 0)
    ...
    speed_values_send = np.array([Motor[1].speed, Motor[0].speed, 0])
    position_values_send = np.array([position_values[1], position_values[0], 0])
    ...
    time_total = t2-t1
    print(f"total time in loop {time_total}")
    ...
    if sleep_time > 0:
        ...
        # No neeed to sleep just go forward
